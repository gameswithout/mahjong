...
| 3.3 Read the tally | Result is Base Win 1 + Concealed 1 + Single Wait 1 = 3 raw Tai. East is the payer, so the flat dealer multiplier applies. Tutorial transfer is 0 Jade; the hypothetical Bamboo payment is `10 × (1 + 3) × 2 = 80` Jade. | Expand the three hand-pattern rows, settlement calculation, and practice/no-transfer row; choose Complete | "Raw hand Tai and settlement modifiers are shown separately. Tutorial never changes Jade." Each unviewed row receives focus; completion remains available after all required rows are viewed. |
...
Playing style is a separate dimension from difficulty. Difficulty answers how accurately a bot executes its plan; a **persona** answers what plan it prefers and what risks it accepts. The two are orthogonal: a persona reweights the same legal action set at every difficulty, and a lower difficulty degrades execution of the chosen style rather than reverting the bot to a different one. A persona is a set of bounded utility preferences — it never overrides legality, never widens the Section 11.2 observation boundary, and never passes a legal Win. The seeded 5% style offset above survives as micro-variation *within* a persona, so two bots of the same persona at one table are still not identical; the persona's own influence is not capped by it.

Personas are available in AI Practice and bot-filled private rooms. They are not offered in public matchmaking or ranked play, and a disconnect takeover always plays the neutral Medium policy with no persona, because the seat's owner chose no style for it. Bots remain visibly labeled as bots regardless of persona, and a persona never changes reaction-time bounds — a style must be readable from decisions, never from response timing.

AI decisions must be deterministic from rules version, AI version, difficulty, persona and persona version, complete legal observation, and a recorded bot-randomness seed. Production uses controlled random variation within the deterministic seed. Decision calculation has a 250 ms server budget; timeout falls back to the Medium legal policy, then canonical auto-discard if needed — a timed-out persona decision falls back to that neutral policy rather than to a partially computed style. Replays retain the AI version, persona version, and seed; executable behavior or an equivalent decision fixture remains available for at least the 180-day raw-match retention window.
