...
1. **AGS capability-mapping spike (P0, scope per D13/Section 2.2a):** confirm per service that AGS/Extend semantics meet the spec's contracts. Checklist: claim/hidden-information boundaries; token lifetimes/revocation (§15.10); Legal-service age-gate fit (§10.3); **Extend match-runtime hosting** — command→Lobby-push latency vs §15.5, append-before-ack durability, app-owned storage provisioning (Stash precedent); `extend-core-matchmaker` fork feasibility for §8.5; **Wallet hold/escrow + idempotency** → wallet-as-authority vs journal-authoritative-with-mirror; Achievement recompute/revocation (§12.3); Challenge determinism, claim windows, catalog governance (§13.3) → native vs Challenge Suite fork; Rank Suite fork vs MMR Calculator; Leaderboard 12-week cycles, region boards, provisional labels (§12.8); inbox claim idempotency, per-item Claim All, expiry (§10.9); Game Telemetry field controls + export (§15.3); reporting category/evidence fit (§10.8); deletion/export coverage vs §10.4. Each line gets a written go/fallback verdict; fallbacks are the 2.2/Step 8 first-party designs.
   - **First pass complete 2026-07-18:** see [AGS capability-mapping spike](ags-plans/2026-07-18-ags-capability-mapping-spike.md) for all 12 verdicts. Headline results: the Wallet-authority question is settled (journal-authoritative — AGS Wallet has no hold/escrow primitive); the rank fork preference should flip from `extend-rank-suite` to `extend-mmr-calculator` (the suite is weekly-batch/tiered, not per-match Elo — this is a correction to §2.2a/E13.F1, not yet applied below); the age-gate needs a Product Owner call (AGS has no month/year-only DOB field); and the report-comment field caps at 280 chars against the spec's 500. Five other items are open follow-ups (see the spike doc's "Follow-ups" section) — none blocking, but not yet resolved.
...
| Rating + seasons (§12.4–12.6) | **Fork `extend-mmr-calculator`** (single Service Extension, already invoked per match result with teams/players/winning team, already Statistics-integrated): replace its ELO variant with the exact §12.4 formula (K 40/24, integer largest-remainder zero-sum, 500 floor redistribution) plus §12.5 season lifecycle. **[CORRECTED 2026-07-18, capability-mapping spike]** — `extend-rank-suite` was the original preference but is architecturally a weekly-batch, 5-tier, 20-player-synthetic-group system (evaluated every Sunday), not a per-match rating engine; retrofitting it to immediate per-match Elo would mean gutting its rank-manager and group-formation logic rather than swapping a formula. See [the spike doc](ags-plans/2026-07-18-ags-capability-mapping-spike.md), item 9. | Fork + modify | First-party Go Elo module |
...
| Rank Suite (api/manager/stats-listener) | Service Extension + Event Handler (Go) | **Rejected for V1** — weekly-batch/tiered/group architecture doesn't fit per-match pairwise Elo (see mapping, corrected 2026-07-18); revisit only if a future weekly-tier ladder mode is added |
| MMR Calculator | Service Extension (Go) | **Fork** — per-match invocation shape matches §12.4 pairwise Elo (see mapping, corrected 2026-07-18) |
...
| E1.F8 | Scoring table | All §6.1 patterns + §6.2 combination/exclusion rules; 69-Tai max scoring fixture; settlement cap remains separate (§5.12). | E1.F7 | L | Rule-interpretation errors | Every §6.1 row has ≥ 1 positive + ≥ 1 negative golden; §6.2 exclusions enumerated as fixtures; Rules Lead sign-off. |
| E1.F9 | Settlement math | Ruleset-selected payer model (discard/Zimo/rob/instant-win), base and Tai components, flat dealer multiplier, debit cap, largest-remainder proportional allocation, exhaustive-draw/dealer-continuation outcomes incl. k-cap 10 (§5.11, §7.3). | E1.F8 | M | Integer-rounding disputes | All §7.4 worked examples reproduce exactly; conservation property (credits == debits) fuzz-tested; signed-64 only. |
...
| E4.F3 | Email/password account (revised per D8, 2026-07-19) | AGS IAM native `EMAILPASSWD` auth: request-code → register (email, username, password, country, DOB month/year+age-confirmation) → password-grant login, client-side only (no first-party token service). Guest→account migration is built as an in-place *headless-account upgrade* (request-code with `upgradeHeadlessAccount` context → `users/me/headless/code/verify`), offered at the end of every match, so the AGS userId and everything keyed to it survive; the settings-conflict summary (§9.10, §10.2) remains a follow-up. | E4.F2 | M | Registration/verification UX; settings-conflict summary deferred | Registration + verification-code + login flow succeeds against AGS IAM in staging; guest upgrade keeps the same userId and Jade balance; under-13 blocked client-side before submission. |
...
| ID | Feature | Objective / description | Deps | Size | Risks | Acceptance criteria |
| --- | --- | --- | --- | --- | --- | --- |
| E13.F1 | Pairwise Elo | Fork `extend-mmr-calculator` (D13, corrected 2026-07-18 per capability-mapping spike item 9) — single Service Extension already invoked per match result and Statistics-integrated; replace its ELO variant with the §12.4 formula, K 40/24, integer largest-remainder zero-sum, 500 floor redistribution, abandonment adjustment separate from Elo (§12.6). `extend-rank-suite` was ruled out: its weekly-batch/tiered/group architecture is a poor fit for per-match rating. | E2.F7, D13 spike | M | Rounding edge cases | All §12.4 worked examples exact; zero-sum property fuzzed; floor cases golden. |
