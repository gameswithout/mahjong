...
const mod = await import("data:text/javascript," + encodeURIComponent(
  src.slice(start, end) + "\nexport { chooseDiscard, chooseClaim };"
));
...
check("pong over chow",      chooseClaim({ can_pong: true, chow_sets: [["a","b"]] }).type, "pong");
check("chow when only one",  chooseClaim({ chow_sets: [["a","b"]] }).type, "chow");
...
const chow = chooseClaim({ chow_sets: [["x1","x2"]] });
check("carries the chow tiles", chow.tile_ids.join(","), "x1,x2");
